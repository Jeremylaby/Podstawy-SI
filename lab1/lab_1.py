...
categorical_pipeline = Pipeline(steps=[("one_hot_encoder",one_hot_encoder)])# your_code_here

numerical_pipeline = Pipeline(steps=[("median_imputer",median_imputer),("min_max_scaler",min_max_scaler)])  # your_code_here
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# // skomentuj tutaj
# Dla naszego modelu następuje lekkie przeuczenie błąd dla zbioru treningowego jest nieco niższy niż dla zbioru treningowego
# Jeśli chodzi o moją subiektywną ocenę błędu ceny mieszkań to nie jest on zbyt wysoki szczególnie jeśli patrzymy na droższe mieszkania czyli to ten błąd wynosi od 10% do 5% ceny (200 000 - 400 000)
# Ale dla tych tańszych wzrasta nawet do 20% ceny miesszkania (około 100 000)
# Więc nasz model nadaje się bardziej do estymacji ceny tych droższych nieruchomości
#
...
print(f"alpha (ridge): {reg_ridge_alpha:.4f}")
print(f"alpha (lasso): {reg_lasso_alpha:.4f}")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# // skomentuj tutaj
# Tak, regularyzacja Ridge i Lasso pomogła zredukować problem overfittingu – małe różnice między wynikami na zbiorze treningowym i testowym świadczą o dobrej generalizacji.
# Zarówno Ridge, jak i Lasso osiągnęły podobny poziom błędu, przy czym Ridge nieco lepiej dopasował się do danych treningowych niż Lasso.
# Mimo lekkiej rozbieżności błędów między zbiorami, oznaki overfittingu są znacznie mniejsze niż w poprzednim podejściu. 
# Dodatkowo wartości błędów na zbiorach testowych są niższe niż wcześniej, co potwierdza poprawę modelu.
#
...
for column in df.select_dtypes(include=['object']).columns:
    print(f"Wartości w kolumnie '{column}':")
    print(df[column].value_counts())
    print("\n")
# your_code
#zastąpiłem każdą wartość modą ewentualnie dla job można by zostawić etykietę unkown
# natomiast w martial można by usunąć rekordy unknown bo jest ich bardzo mało
replace_na(df,"loan",value="no")
replace_na(df,"job",value="admin.")
replace_na(df,"education",value="secondary")
replace_na(df,"housing",value="yes")
replace_na(df,"marital",value="married")
...
for col in categorical_columns:
    ...
    counts.name = col
    class_counts = pd.concat([class_counts, counts])
ax = class_counts.plot.bar(figsize=(20, 8))

ax.set_title(f'Częstość klas w kolumnie')
ax.set_xlabel('Klasa')
...
#nol
logistic_model = LogisticRegression(class_weight="balanced",penalty=None,random_state=0)
logistic_model.fit(X_train,y_train)
logistic_pred = logistic_model.predict(X_test)
nol_precision = precision_score(y_true=y_test,y_pred=logistic_pred)
print(f"nol_precission: {nol_precision*100:.2f}%")
nol_recall = recall_score(y_true=y_test,y_pred=logistic_pred)
print(f"nol_recall: {nol_recall*100:.2f}%")
nol_f1 = f1_score(y_true=y_test,y_pred=logistic_pred)
print(f"nol_f1: {nol_f1*100:.2f}%")
#l2
logistic_cv_model = LogisticRegressionCV(Cs=100,cv=5,scoring="f1",penalty="l2",
                                         solver="lbfgs",class_weight="balanced",random_state=0,n_jobs=-1)
logistic_cv_model.fit(X_train,y_train)
logistic_cv_pred = logistic_cv_model.predict(X_test)
l2_precision = precision_score(y_true=y_test,y_pred=logistic_cv_pred)
print(f"l2_precission: {l2_precision*100:.2f}%")
l2_recall = recall_score(y_true=y_test,y_pred=logistic_cv_pred)
print(f"l2_recall: {l2_recall*100:.2f}%")
l2_f1 = f1_score(y_true=y_test,y_pred=logistic_cv_pred)
print(f"l2_f1: {l2_f1*100:.2f}%")
#l1
logistic_cv1_model = LogisticRegressionCV(Cs=100,cv=5,scoring="f1",penalty="l1",solver="saga",class_weight="balanced",random_state=0,n_jobs=-1)
logistic_cv1_model.fit(X_train,y_train)
logistic_cv1_pred = logistic_cv1_model.predict(X_test)
l1_precision = precision_score(y_true=y_test,y_pred=logistic_cv1_pred)
print(f"l1_precission: {l1_precision*100:.2f}%")
l1_recall = recall_score(y_true=y_test,y_pred=logistic_cv1_pred)
print(f"l1_recall: {l1_recall*100:.2f}%")
l1_f1 = f1_score(y_true=y_test,y_pred=logistic_cv1_pred)
print(f"l1_f1: {l1_f1*100:.2f}%")
...
# %% editable=true slideshow={"slide_type": ""} tags=["ex"]
# your_code
logistic_no_reg_model = LogisticRegression(class_weight='balanced', penalty=None, random_state=0)
logistic_no_reg_model.fit(X_train, y_train)
y_train_pred = logistic_no_reg_model.predict(X_train)
y_test_pred = logistic_no_reg_model.predict(X_test)
f1_train = f1_score(y_true=y_train, y_pred=y_train_pred)
f1_test = f1_score(y_true=y_test, y_pred=y_test_pred)
print(f"Miara F1 na zbiorze treningowym: {f1_train*100:.2f}")
print(f"Miara F1 na zbiorze testowym: {f1_test*100:.2f}")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# // skomentuj tutaj
# Dla naszego modelu ważniejszy jest recall, ponieważ zależy nam na dotarciu do wszystkich klientów potencjalnie zainteresowanych lokatą. Chcemy zminimalizować liczbę przypadków, w których klient, który mógłby być zainteresowany ofertą, zostałby oznaczony jako niezainteresowany (fałszywy negatyw).
#
# Jednak precision również odgrywa istotną rolę, ponieważ pozwala obniżyć koszty kampanii marketingowej. Dzięki wysokiej precision możemy ograniczyć liczbę wyświetleń reklamy klientom, którzy rzeczywiście nie są zainteresowani lokatą. Gdyby nie wysoka wartość precision, reklama mogłaby być wyświetlana niemal każdemu, co zniwelowałoby potrzebę stosowania modelu.
#
# Aby zmienić stosunek tych waartości wystarczy zwiększyć lub zmnienić proporcję negatywnych i pozytywnych przypadków w naszej bazie danych 
# Zwięksenie przypadków negatywnych może zwiększyć precyzję naszego modelu
# Możemy też zmienić te proporcje po przez zmiane classification tresurehold z domyślnego 0.5 na inną wartość to może "zachęcić" model żeby częsciej decydować się na odpowiedź pozytywną lub negatywną
...
X_train = column_transformer.fit_transform(X_train)
X_test = column_transformer.transform(X_test)
logistic_model =LogisticRegression(penalty=None, class_weight="balanced", random_state=0, max_iter=1000)
...
print("Wartości f1 bez regularyzacji: ",f1_train,f1_test)
...
# %% editable=true slideshow={"slide_type": ""} tags=["ex"]
# TODO solution
#zadanie działa pod warunkiem że wykonamy wcześniej wszystkie operacje związane z przetwarzaniem
#danych zbioru ames housing
#jednak coś nie działa próbowałem :(((((
...
ridge_model = sm.OLS(y_train, X_train).fit_regularized(alpha=alpha_l2, L1_wt=0)  # L2
lasso_model = sm.OLS(y_train, X_train).fit_regularized(alpha=alpha_l1, L1_wt=1)  # L1

print("Test Harvey-Colliera dla modelu OLS:", linear_harvey_collier(results).pvalue)
print("Test Harvey-Colliera dla Ridge:", linear_harvey_collier(ridge_model).pvalue)
print("Test Harvey-Colliera dla Lasso:", linear_harvey_collier(lasso_model).pvalue)
#https://youtu.be/eVTXPUF4Oz4?si=GSq6kGh7a0erJzzu&t=55
